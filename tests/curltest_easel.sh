...
curl --silent -X GET https://10.100.201.3:12300/hello --insecure > actual.txt
# cat actual.txt
if diff ./tests/expected/01_hello_result.txt actual.txt > /dev/null; then
  ...
  #fail 
  echo "Pass Test 01" # if TA changes hello endopoint and test fails, 
                      # they might not look here to see why :)
...
curl --silent -X 'POST' \
  'https://10.100.201.3:12300/properties' \
...
# Test 03 Get Property 2
curl --silent -X 'GET' \
  'https://10.100.201.3:12300/properties/2' \
  -H 'accept: application/json' --insecure > actual.txt
# cat actual.txt
if diff ./tests/expected/03_get_id2_adminer.txt actual.txt > /dev/null; then
  # pass
  echo "Pass Test 03"
else 
  #fail 
...
curl --silent -X 'GET' \
  'https://10.100.201.3:12300/properties/0' \
  -H 'accept: application/json' --insecure > actual.txt
# cat actual.txt
